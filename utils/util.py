...
from .utils_y import get_host_name
...
def mkdir_and_rename(path):
    if os.path.exists(path):
        ...
        print(f'Path already exists. Rename it to {new_name}, Will make dirs {os.path.realpath(path)}, path={path}, aaa!')
        logger = logging.getLogger('inrlog')
        logger.info(f'Path already exists. Rename it to {new_name}, Will make dirs {os.path.realpath(path)}, path={path}!')
        # logger.info(f'Will make dirs {os.path.realpath(path)}!')
...
def setup_logger(logger_name, root, phase, level=logging.INFO, screen=False, tofile=False):
    ...
    lg.setLevel(level)
    if tofile:
        log_file = os.path.join(root, phase + f'_{get_timestamp()}_{get_host_name()}.log')
