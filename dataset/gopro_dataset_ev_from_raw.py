...
class goproDataset_ev_from_raw(data.Dataset):
    '''
    ...
    def __init__(self, opt):
        super(goproDataset_ev_from_raw, self).__init__()
        self.opt = opt
        ...
        if self.opt['is_train']: #   : true
            ...
            self.hq_scale = 1
            self.lq_scale = self.hq_scale * (1 /self.spatial_upscale)
            print(f"==>> In class goproDataset_ev_from_raw self.spatial_upscale: {self.spatial_upscale}")


        if self.opt.get('is_test', False):  # dict get 防止键不存在  #opt['is_test']==True:
            # num_imgs = 48  # 49  # 00000.png ~ 00048.png
        ...
        for video in video_list:
            ...
            # frames = sorted([int(frame[:-4]) for frame in frames if frame.endswith('.png') ])
            ...
            while index + self.num_midpoints * 1 + 1 <= num_imgs-1:   #  99 0~91
                ...
                self.ev_npz_llist.append(tmp1)
                index += 1

        if self.opt.get('is_test', False):
            self.file_list = self.file_list[::(self.num_midpoints+1)]   # 8
            self.gt_list = self.gt_list[::(self.num_midpoints+1)]
            self.ev_npz_llist = self.ev_npz_llist[::(self.num_midpoints+1)]
    ...
    def _load_img(self,index,gt_sampled_idx_l,lq_scale,hq_scale):
        ...
        img_lq_l = [(cv2.imread(fp)[:self.h_new,:self.w_new,:]/255.) for fp in img_lq_path_l] # List[np.ndarray] 2  (624, 960, 3)
        img_gt_l = [(cv2.imread(fp)[:self.h_new,:self.w_new,:]/255.) for fp in img_gt_path_l] # List[np.ndarray] 9  (624, 960, 3)
    ...
    def _load_event(self,ev_n4_l,gt_sampled_idx,h_ori,w_ori,lq_scale):
        ...
        #* crop to 960,1440
        ev_0t_voxel_hr = ev_0t_voxel_hr[:,:self.h_new,:self.w_new]
        ev_t1_voxel_hr = ev_t1_voxel_hr[:,:self.h_new,:self.w_new]
    ...
    def __getitem__(self, index):
        ...
        sce_name_list   = [ f.split('/')[0]  for f in self.gt_list[index]]
